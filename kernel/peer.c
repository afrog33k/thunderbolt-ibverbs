...
void tbv_peer_remove_rail(struct tbv_rail *rail)
{
	...
	/*
	 * Quiesce before unregister, free after destroy. ib_unregister_device()
	 * drives destroy_qp for every QP on this rail and blocks until they
	 * return; a destroy can only return once every descriptor submitted on
	 * the QP's behalf has retired. With the ring stop living behind the
	 * unregister (in tbv_path_destroy()) a stuck descriptor made that a
	 * deadlock resolved by a timeout. Retire the DMA first, with the ring
	 * barrier as the fallback, so the destroys find nothing in flight.
	 */
	tbv_apple_path_quiesce(rail, NULL, "rail_remove",
