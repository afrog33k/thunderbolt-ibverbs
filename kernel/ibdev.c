...
/*
 * Three things were one knob. Confirmed from this tree, 2026-09-06:
 *
 *  - With apple_tx_pipeline=N, tbv_apple_tx_exclusive() returns
 *    tbv_apple_tx_requires_exclusive_window(frames), which is simply
 *    frames > 0 (proto/apple_tx.h). tbv_apple_tx_window_ok() then refuses
 *    admission whenever cur_wr or cur_frames is non-zero, so EVERY non-empty
 *    SEND takes an exclusive per-QP window: exactly one message is in flight
 *    per QP and application queue depth only queues. apple_tx_max_inflight_wr
 *    is not consulted at all in that mode.
 *
 *  - With apple_tx_pipeline=Y, each WR charges
 *    tbv_apple_tx_frame_charge(frames, max_frames) = min(frames, max_frames)
 *    against the same max_frames -- and max_frames was ALSO the intra-message
 *    group size (group_limit in tbv_apple_sq_transmit). So any message at
 *    least as large as the group consumes the entire admission budget, and a
 *    second WR can never be admitted no matter how large
 *    apple_tx_max_inflight_wr is. Raising the WR limit alone cannot create
 *    overlap. That is why the 4 KiB row of the matrix falls from
 *    1409 to 732 Mbit/s as the group grows (g4 -> g32 at depth 8) even though
 *    a 4 KiB message is a single frame and the group gate cannot apply to it:
 *    the same number was throttling admission, not batching publication.
 *
 * Split them, with 0 meaning "follow apple_tx_max_inflight_frames" so the
 * default is byte-identical to the coupled behaviour that produced the matrix.
 */
...
MODULE_PARM_DESC(apple_tx_max_inflight_wr,
		 "WR concurrency: maximum Apple-compatible UC SENDs in flight per QP. Only consulted with apple_tx_pipeline=1; with the default serialized mode every non-empty SEND takes an exclusive per-QP window and this value is ignored");
...
MODULE_PARM_DESC(apple_tx_max_inflight_frames,
		 "Legacy combined knob: sets BOTH the intra-message publication group and the per-QP outstanding-frame budget. Override either independently with apple_tx_group_frames / apple_tx_budget_frames; 0 disables macOS receiver backpressure");

static uint apple_tx_group_frames;
module_param(apple_tx_group_frames, uint, 0644);
MODULE_PARM_DESC(apple_tx_group_frames,
		 "Batch size: frames published before the SQ worker waits for the previous group to drain. Each internal boundary costs one hardware round trip (~40 us measured). 0 = follow apple_tx_max_inflight_frames (default)");

static uint apple_tx_budget_frames;
module_param(apple_tx_budget_frames, uint, 0644);
MODULE_PARM_DESC(apple_tx_budget_frames,
		 "Admission budget: total outstanding frames a QP may hold. Each WR charges min(message_frames, group) against it, so this must exceed the group before apple_tx_max_inflight_wr can create any overlap. 0 = follow apple_tx_max_inflight_frames (default)");

/* Publication batch size. */
static u32 tbv_apple_tx_group(void)
{
	u32 group = READ_ONCE(apple_tx_group_frames);

	return group ? group : READ_ONCE(apple_tx_max_inflight_frames);
}

/* Per-QP outstanding-frame admission budget. */
static u32 tbv_apple_tx_budget(void)
{
	u32 budget = READ_ONCE(apple_tx_budget_frames);

	return budget ? budget : READ_ONCE(apple_tx_max_inflight_frames);
}

/*
 * Pipelining experiment (2026-09-05). With the exclusive per-QP window every
 * Apple-compatible SEND waits for its own TX completion before the next WR is
 * posted, and on a stock kernel the NHI signals TX completions behind the
 * upstream 128 us interrupt throttle (nhi_enable_int_throttling). Measured
 * Strix Halo -> M4 Max: a flat ~7,650 WR/s at every size, i.e. 0.25 Gbit/s at
 * 4 KiB, 1.0 at 16 KiB, 2.0 at 32 KiB, unchanged by apple_tx_max_inflight_frames
 * (4..1024) and by nhi_interrupt_throttle_ns (inert without tb_ring_throttling).
 *
 * apple_tx_pipeline=1 drops the exclusive window so up to apple_tx_max_inflight_wr
 * WRs are in flight per QP. Frame order on the ring is still per-WR contiguous
 * because a QP's Apple SQ is drained by one work item that posts every frame of a
 * WR before popping the next; completions stay ordered via
 * tbv_apple_complete_ordered_sends().
 */
static bool apple_tx_pipeline;
module_param(apple_tx_pipeline, bool, 0644);
MODULE_PARM_DESC(apple_tx_pipeline,
		 "Allow up to apple_tx_max_inflight_wr Apple-compatible UC SENDs in flight per QP instead of one; default off (serialized)");

/*
 * SOF marking on Apple-compatible frames. Mode 0 is the historical Linux
 * behaviour (SOF on every frame). macOS itself marks SOF on the first frame of
 * a multi-frame message only, and emits single-frame SENDs as EOF=3 with no SOF
 * (see tbv_ibdev_rx_apple_frame). Mode 1 = SOF on the first frame only, mode 2 =
 * mirror macOS exactly.
 */
static uint apple_tx_sof_mode;
module_param(apple_tx_sof_mode, uint, 0644);
MODULE_PARM_DESC(apple_tx_sof_mode,
		 "SOF marking on Apple-compatible frames: 0 every frame (default), 1 first frame only, 2 macOS-style (first frame of multi-frame messages only)");

static u8 tbv_apple_tx_sof(u32 offset, u32 nfrags)
{
	switch (READ_ONCE(apple_tx_sof_mode)) {
	case 1:
		return offset == 0 ? 1 : 0;
	case 2:
		return (offset == 0 && nfrags > 1) ? 1 : 0;
	default:
		return 1;
	}
}

static bool tbv_apple_tx_exclusive(u32 frames)
{
	if (READ_ONCE(apple_tx_pipeline))
		return false;
	return tbv_apple_tx_requires_exclusive_window(frames);
}
...
struct tbv_qp {
	...
	/* Rail binding epoch this QP owns (Apple only); see struct tbv_rail_bind. */
	u64 bind_epoch;
	bool rail_bound;
	bool qpn_allocated;
...
struct tbv_send_ctx {
	...
	struct tbv_tx_stamps tx_stamps;
...
static int tbv_rail_apple_admit(struct tbv_rail *rail, u32 local_qpn,
				const char *what);
static void tbv_qp_apple_unbind_rail(struct tbv_qp *tqp);
static void tbv_qp_stop_producers(struct tbv_qp *tqp);
static int tbv_qp_wait_refs(struct tbv_qp *tqp, unsigned int target,
...
/*
 * The QP that owns the rail's FA57 data path: an Apple UC QP. A GSI QP on an
 * Apple ib_device (none exists today, see "Couldn't create ib_mad QP1") must
 * never bind, quiesce or close the shared path.
 */
static bool tbv_qp_owns_apple_rail(const struct tbv_qp *tqp)
{
	return tbv_qp_uses_apple_transport(tqp) && tqp->type == IB_QPT_UC;
}
...
static bool tbv_rail_apple_bound_to(struct tbv_rail *rail, struct tbv_qp *tqp)
{
	unsigned long flags;
	bool bound;

	spin_lock_irqsave(&rail->bind_lock, flags);
	bound = rail->bind.state == TBV_RAIL_BOUND && rail->bind.qp == tqp &&
		rail->bind.epoch == tqp->bind_epoch;
	spin_unlock_irqrestore(&rail->bind_lock, flags);
	return bound;
}
...
tbv_first_active_apple_path_for_qp_locked(struct tbv_qp *tqp)
{
	...
	if (rail->peer->backend != TBV_BACKEND_APPLE)
		return NULL;
	/*
	 * The path is selected from the QP's rail, never derived from the
	 * remote QPN. That is only sound while this QP's epoch owns the rail;
	 * once the rail is quiescing or was rebound, refuse rather than push
	 * frames of a retired epoch down the wire.
	 */
	if (tbv_qp_owns_apple_rail(tqp) && !tbv_rail_apple_bound_to(rail, tqp)) {
		atomic64_inc(&tqp->owner->apple_tx_unbound);
		return NULL;
	}
			 ...
			 struct ib_udata *udata)
{
	...
	mutex_unlock(&state->lock);
	if (tbv_backend_is_apple(tqp->backend) && !gsi) {
		/*
		 * Refuse at create, before any resource is touched, while the
		 * previous epoch is still retiring its DMA on this rail.
		 */
		ret = tbv_rail_apple_admit(tqp->rail, 0, "create_qp");
		if (ret)
			goto err_put_rail;
	}
...
static int tbv_destroy_qp(struct ib_qp *qp, struct ib_udata *udata)
{
	...
	if (tbv_qp_owns_apple_rail(tqp) && tqp->rail) {
		/*
		 * The Apple QP owns its rail's path. Every descriptor
		 * submitted on its behalf is retired before the QP can be
		 * freed; a descriptor the NHI never completes is ended by the
		 * ring barrier, not by a timeout that returns -ETIMEDOUT and
		 * leaves the RDMA core holding the QP's GID reference forever
		 * (ib_destroy_qp_user() only releases it after a successful
		 * driver destroy).
		 */
		ret = tbv_apple_path_quiesce(tqp->rail, tqp, "destroy_qp",
					     tbv_ibdev_quiesce_deadline());
		if (ret) {
			pr_err("QP %u destroy refused: rail %u retirement barrier failed (%d) with %u refs and %d unretired TX frames; the QP stays closing so no late callback can reach freed memory\n",
			       qp->qp_num, tqp->rail->rail_id, ret,
			       refcount_read(&tqp->refs),
			       tbv_path_unretired_tx(&tqp->rail->path));
			return ret;
		}
	} else {
		tbv_qp_stop_producers(tqp);
		if (tbv_qp_wait_refs(tqp, 1, tbv_ibdev_quiesce_deadline())) {
...
/*
 * Rail binding (Apple only). All of these run with rail->bind_lock held by the
 * caller unless the name says otherwise. Lock order: state->lock (mutex) ->
 * rail->bind_lock -> tqp->lock; never take bind_lock under tqp->lock.
 */

static u32 tbv_apple_remote_qpn_hop(u32 remote_qpn)
{
	return remote_qpn >> TBV_APPLE_QPN_SHIFT;
}

/*
 * Admission for a NEW binding. Returns 0 when the rail is UNBOUND, -EBUSY
 * with a teaching error otherwise. BOUND is the caller's business.
 *
 * A QUARANTINED rail (a teardown needed the ring-stop barrier, or a TX stall
 * failed the connection) is lifted only by a NEW PEER SESSION: a bind that
 * arrives with a remote QPN (@new_session, i.e. modify to RTR), never by
 * create_qp, and only while the peer has been observed silent since the
 * barrier and nothing is unretired. Local silence cannot prove the peer's
 * reassembly state or that the fabric is clean; the fresh binding generation
 * is the boundary we can insist on. Every refusal prints the numbers.
 */
static int tbv_rail_apple_admit_locked(struct tbv_rail *rail, u32 local_qpn,
				       const char *what, bool new_session)
{
	struct tbv_rail_bind *b = &rail->bind;
	u64 rx_now;
	int submitted;
	...
	switch (b->state) {
	case TBV_RAIL_UNBOUND:
		return 0;
	case TBV_RAIL_BOUND:
		return 0;
	case TBV_RAIL_QUIESCING:
		b->bind_refused++;
		pr_warn_ratelimited("apple %s refused: rail %u is still quiescing epoch %llu (local_qpn=%u remote_qpn=%u submitted=%d); qpn %u must wait until peers shows bind state=unbound\n",
				    what, rail->rail_id, b->epoch, b->local_qpn,
				    b->remote_qpn,
				    atomic_read(&rail->path.tx_inflight),
				    local_qpn);
		return -EBUSY;
	case TBV_RAIL_QUARANTINED:
		rx_now = atomic64_read(&rail->path.data_rx_completed);
		submitted = atomic_read(&rail->path.tx_inflight);
		if (!new_session) {
			b->bind_refused++;
			pr_warn_ratelimited("apple %s refused: rail %u quarantined (%s, epoch %llu, %u ms ago); only a new peer session (modify to RTR with a remote QPN) can lift it\n",
					    what, rail->rail_id,
					    b->quarantine_reason,
					    b->quarantine_epoch,
		...
		if (rx_now == b->quarantine_rx_mark && !submitted &&
		    !rail->path.rings_stopped && !b->last_teardown_error) {
			pr_info("apple rail %u quarantine (%s, epoch %llu) lifted by a new peer session after %u ms: no peer frames since the ring barrier, no unretired TX; local retirement only, peer reassembly state is not proven\n",
				rail->rail_id, b->quarantine_reason,
				b->quarantine_epoch,
				jiffies_to_msecs(jiffies - b->quarantine_since));
			WRITE_ONCE(b->state, TBV_RAIL_UNBOUND);
			/* bind_lock -> tx_lock; never the other way round. */
			tbv_path_open_tx(&rail->path);
			return 0;
		}
		b->bind_refused++;
		pr_warn_ratelimited("apple %s refused: rail %u quarantined (%s, epoch %llu) for %u ms (last teardown %s err=%d): %llu peer frames since the ring barrier, %d TX frames unretired, rings_stopped=%u; the peer has not drained, retry later\n",
				    what, rail->rail_id, b->quarantine_reason,
				    b->quarantine_epoch,
				    jiffies_to_msecs(jiffies - b->quarantine_since),
				    b->last_teardown_reason, b->last_teardown_error,
				    rx_now - b->quarantine_rx_mark, submitted,
				    rail->path.rings_stopped);
		/* Re-arm: the next attempt must again observe a silent peer. */
		b->quarantine_rx_mark = rx_now;
		b->quarantine_since = jiffies;
		return -EBUSY;
	}
	return -EINVAL;
}

/* create_qp admission: refuse only while a previous epoch is still retiring. */
static int tbv_rail_apple_admit(struct tbv_rail *rail, u32 local_qpn,
				const char *what)
{
	unsigned long flags;
	...
	spin_lock_irqsave(&rail->bind_lock, flags);
	ret = rail->bind.state == TBV_RAIL_QUARANTINED ?
		      0 :
		      tbv_rail_apple_admit_locked(rail, local_qpn, what, false);
	spin_unlock_irqrestore(&rail->bind_lock, flags);
	if (ret == -EBUSY && rail->peer && rail->peer->state)
		atomic64_inc(&rail->peer->state->apple_bind_refused);
	return ret;
}

/*
 * Validate the remote QPN against the path this rail actually transmits on.
 * An Apple FA57 QPN encodes the owner's receive HopID in its upper bits (our
 * own is receive_path << TBV_APPLE_QPN_SHIFT, see tbv_apple_qpn_from_path());
 * the peer's receive hop is the hop our tunnel was programmed to transmit to.
 * A peer that re-enumerated its tunnel shows up as a QPN whose hop no longer
 * matches local_transmit_path, and no amount of storing the new number makes
 * this path reach it.
 */
static int tbv_rail_apple_check_remote_qpn_locked(struct tbv_rail *rail,
						  u32 remote_qpn)
{
	struct tbv_path *path = &rail->path;
	u32 hop = tbv_apple_remote_qpn_hop(remote_qpn);

	if (path->local_transmit_path < 0 ||
	    hop == (u32)path->local_transmit_path)
		return 0;

	rail->bind.bind_refused++;
	pr_warn_ratelimited("apple bind refused: remote QPN %u encodes peer receive hop %u but rail %u transmits on hop %d (epoch %llu); the peer re-enumerated its tunnel and this path cannot reach it\n",
			    remote_qpn, hop, rail->rail_id,
			    path->local_transmit_path, rail->bind.epoch);
	return -EINVAL;
}

/*
 * Bind (or re-validate) the QP's rail on a transition into a transport state.
 * Idempotent for the epoch that already owns the rail; refuses a changed remote
 * QPN, a foreign owner, a quiescing or quarantined rail, and a remote QPN whose
 * path does not match.
 */
static int tbv_qp_apple_bind_rail(struct tbv_qp *tqp, bool remote_known,
				  u32 remote_qpn)
{
	struct tbv_rail *rail = tqp->rail;
	struct tbv_rail_bind *b;
	unsigned long flags;
	u32 qpn = tqp->base.qp_num;
	bool refused = false;
	int ret = 0;

	if (!tbv_qp_owns_apple_rail(tqp) || !rail)
		return 0;

	b = &rail->bind;
	spin_lock_irqsave(&rail->bind_lock, flags);
	if (tqp->rail_bound) {
		if (b->state != TBV_RAIL_BOUND || b->qp != tqp ||
		    b->epoch != tqp->bind_epoch) {
			b->bind_refused++;
			refused = true;
			pr_warn_ratelimited("apple bind refused: qpn %u epoch %llu no longer owns rail %u (state=%s epoch=%llu owner_qpn=%u)\n",
					    qpn, tqp->bind_epoch, rail->rail_id,
					    tbv_rail_bind_state_name(b->state),
					    b->epoch, b->local_qpn);
			ret = -EBUSY;
			goto out;
		}
		if (remote_known && b->remote_qpn_known &&
		    b->remote_qpn != remote_qpn) {
			b->bind_refused++;
			refused = true;
			pr_warn_ratelimited("apple bind refused: remote QPN changed %u -> %u on rail %u epoch %llu; the FA57 path cannot be re-targeted in place, destroy the QP and let the rail quiesce\n",
					    b->remote_qpn, remote_qpn,
					    rail->rail_id, b->epoch);
			ret = -EINVAL;
			goto out;
		}
		if (remote_known && !b->remote_qpn_known) {
			ret = tbv_rail_apple_check_remote_qpn_locked(rail,
								     remote_qpn);
			if (ret) {
				refused = true;
				goto out;
			...
			b->remote_qpn = remote_qpn;
			b->remote_qpn_known = true;
		}
		goto out;
	}

	ret = tbv_rail_apple_admit_locked(rail, qpn, "bind", remote_known);
	if (ret) {
		refused = ret == -EBUSY;
		goto out;
	}
	if (b->state == TBV_RAIL_BOUND) {
		b->bind_refused++;
		refused = true;
		pr_warn_ratelimited("apple bind refused: rail %u already bound to qpn %u (epoch %llu); qpn %u cannot share the FA57 path\n",
				    rail->rail_id, b->local_qpn, b->epoch, qpn);
		ret = -EBUSY;
		goto out;
	}
	if (remote_known) {
		ret = tbv_rail_apple_check_remote_qpn_locked(rail, remote_qpn);
		if (ret) {
			refused = true;
			goto out;
		}
	...
	b->epoch++;
	b->state = TBV_RAIL_BOUND;
	b->qp = tqp;
	b->local_qpn = qpn;
	b->remote_qpn = remote_known ? remote_qpn : 0;
	b->remote_qpn_known = remote_known;
	tqp->bind_epoch = b->epoch;
	tqp->rail_bound = true;
	pr_info("apple rail %u bound epoch=%llu local_qpn=%u remote_qpn=%u tx_hop=%d\n",
		rail->rail_id, b->epoch, qpn, b->remote_qpn,
		rail->path.local_transmit_path);
out:
	spin_unlock_irqrestore(&rail->bind_lock, flags);
	if (refused)
		atomic64_inc(&tqp->owner->apple_bind_refused);
	return ret;
}

/*
 * Release the binding without a quiesce. Only correct when nothing of this
 * epoch can still be in flight; the teardown path replaces this with
 * tbv_apple_path_quiesce().
 */
static void tbv_qp_apple_unbind_rail(struct tbv_qp *tqp)
{
	struct tbv_rail *rail = tqp->rail;
	...
	if (!rail || !tqp->rail_bound)
		return;

	spin_lock_irqsave(&rail->bind_lock, flags);
	if (rail->bind.qp == tqp) {
		rail->bind.qp = NULL;
		if (rail->bind.state == TBV_RAIL_BOUND)
			rail->bind.state = TBV_RAIL_UNBOUND;
	}
	tqp->rail_bound = false;
	spin_unlock_irqrestore(&rail->bind_lock, flags);
}

static void tbv_qp_debug_counts_locked(struct tbv_qp *tqp, u32 *sq_queued,
				       u32 *pending_sends)
{
	...
	*sq_queued = 0;
	*pending_sends = 0;
	list_for_each(pos, &tqp->apple_sq)
		(*sq_queued)++;
	list_for_each(pos, &tqp->pending_sends)
		(*pending_sends)++;
}

void tbv_ibdev_debugfs_rail_show(struct seq_file *s, struct tbv_rail *rail)
{
	struct tbv_path_ring_snapshot ring;
	struct tbv_rail_bind b;
	struct tbv_qp *tqp;
	unsigned long flags;
	u32 sq_queued = 0;
	u32 sq_outstanding = 0;
	u32 pending_sends = 0;
	u32 qp_refs = 0;
	int qp_state = -1;
	bool qp_closing = false;
	u32 tx_queued;
	u32 tx_reserved;
	...
	if (tqp) {
		spin_lock(&tqp->lock);
		qp_refs = refcount_read(&tqp->refs);
		qp_state = tqp->state;
		qp_closing = tqp->closing;
		sq_outstanding = tqp->apple_sq_outstanding;
		tbv_qp_debug_counts_locked(tqp, &sq_queued, &pending_sends);
		spin_unlock(&tqp->lock);
	}
	spin_unlock_irqrestore(&rail->bind_lock, flags);

	spin_lock_irqsave(&rail->path.tx_lock, flags);
	tx_queued = rail->path.tx_data_queued + rail->path.tx_control_queued;
	tx_reserved = rail->path.tx_data_reserved;
	spin_unlock_irqrestore(&rail->path.tx_lock, flags);
	tbv_path_tx_ring_snapshot(&rail->path, &ring);

	/*
	 * tx_unretired is the first-class count of descriptors handed to the
	 * NHI whose completion callback has not run (path.tx_inflight) — the
	 * number a stuck TX path shows directly, not a posted-minus-completed
	 * subtraction. The ring_tx_* fields are the NHI ring's own indices:
	 * outstanding = descriptors the hardware owns and software has not
	 * reaped, tail_completed = whether the oldest of them is already
	 * COMPLETED (a missed notification) or not (hardware/transport stall).
	 */
	seq_printf(s,
		   "    bind state=%s epoch=%llu local_qpn=%u remote_qpn=%u remote_qpn_known=%u qp_refs=%u qp_state=%d qp_closing=%u sq_queued=%u sq_outstanding=%u pending_sends=%u tx_queued=%u tx_reserved=%u tx_unretired=%d tx_reaped=%lld tx_closed=%u rings_stopped=%u quiesce_count=%llu ring_stops=%llu bind_refused=%llu last_teardown_error=%d last_teardown_ms=%u last_teardown_reason=%s\n",
		   tbv_rail_bind_state_name(b.state), b.epoch, b.local_qpn,
		   b.remote_qpn, b.remote_qpn_known, qp_refs, qp_state,
		   qp_closing, sq_queued, sq_outstanding, pending_sends,
		   tx_queued, tx_reserved, tbv_path_unretired_tx(&rail->path),
		   atomic64_read(&rail->path.tx_reaped),
		   rail->path.tx_closed, rail->path.rings_stopped,
		   b.quiesce_count, b.ring_stops, b.bind_refused,
		   b.last_teardown_error, b.last_teardown_ms,
		   b.last_teardown_reason);
	seq_printf(s,
		   "    quarantine reason=%s epoch=%llu age_ms=%u rx_mark=%llu\n",
		   b.quarantine_reason, b.quarantine_epoch,
		   b.state == TBV_RAIL_QUARANTINED ?
			   jiffies_to_msecs(jiffies - b.quarantine_since) : 0,
		   b.quarantine_rx_mark);
	seq_printf(s,
		   "    ring_tx valid=%u running=%u head=%d tail=%d hw_cons=%d size=%d outstanding=%d sw_queued=%d in_flight=%d tail_flags=0x%03x tail_completed=%u ring_flags=0x%x e2e_tx_hop=%d hw_options=0x%08x tx_stalled=%u tx_stall_count=%u\n",
		   ring.valid, ring.running, ring.head, ring.tail, ring.hw_index,
		   ring.size, ring.outstanding, ring.sw_queued, ring.in_flight,
		   ring.tail_flags, ring.tail_completed, ring.ring_flags,
		   ring.e2e_tx_hop, ring.hw_options, rail->path.tx_stalled,
		   rail->path.tx_stall_count);
	seq_printf(s,
		   "    tx_stalls count=%u recovered=%u longest_ms=%u total_ms=%llu last_frames=%u\n",
		   rail->path.tx_stall_count, rail->path.tx_stall_recovered,
		   rail->path.tx_stall_longest_ms, rail->path.tx_stall_total_ms,
		   rail->path.tx_stall_frames);
	seq_printf(s,
		   "    tx_stall_recovered_by peer=%u reap=%u rekick=%u timeout=%u rekicks_issued=%u\n",
		   rail->path.tx_stall_recovered_by[TBV_TX_STALL_RECOVERY_PEER],
		   rail->path.tx_stall_recovered_by[TBV_TX_STALL_RECOVERY_REAP],
		   rail->path.tx_stall_recovered_by[TBV_TX_STALL_RECOVERY_REKICK],
		   rail->path.tx_stall_recovered_by[TBV_TX_STALL_RECOVERY_TIMEOUT],
		   rail->path.tx_rekicks);
	/*
	 * The resolved values, not the raw parameters: group and budget both
	 * fall back to apple_tx_max_inflight_frames, and wr is ignored entirely
	 * while pipeline is off. A number without its resolved config is not a
	 * result.
	 */
	seq_printf(s,
		   "    apple_tx pipeline=%u group=%u budget=%u max_wr=%u (legacy_frames=%u group_param=%u budget_param=%u) completion_delay_us=%u\n",
		   READ_ONCE(apple_tx_pipeline), tbv_apple_tx_group(),
		   tbv_apple_tx_budget(),
		   READ_ONCE(apple_tx_pipeline) ?
			   READ_ONCE(apple_tx_max_inflight_wr) : 1,
		   READ_ONCE(apple_tx_max_inflight_frames),
		   READ_ONCE(apple_tx_group_frames),
		   READ_ONCE(apple_tx_budget_frames),
		   READ_ONCE(apple_tx_completion_delay_us));
}

/*
 * Quiesce bounds. DRAIN caps how long a teardown lets already-submitted
 * descriptors finish before the ring barrier (see tbv_apple_path_quiesce());
 * GRACE bounds the reference wait after a barrier, when no ring callback can
 * be pending anymore.
 */
#define TBV_QUIESCE_DRAIN_MS 100
#define TBV_QUIESCE_BARRIER_GRACE_MS 1000

unsigned long tbv_ibdev_quiesce_deadline(void)
{
	uint ms = READ_ONCE(qp_timeout_ms);

	return jiffies + msecs_to_jiffies(ms ? ms : TBV_QP_TIMEOUT_DEFAULT_MS);
}
			    ...
			    unsigned long deadline)
{
	...
	if (refcount_read(&tqp->refs) <= target)
		return 0;
	left = (long)deadline - (long)jiffies;
	if (left <= 0)
		return -ETIMEDOUT;
	wait_event_timeout(tqp->refs_wait, refcount_read(&tqp->refs) <= target,
			   left);
	return refcount_read(&tqp->refs) <= target ? 0 : -ETIMEDOUT;
}

/*
 * Stop everything that can still submit or re-arm on behalf of this QP, and
 * fail every WR it still owns. Submitted descriptors keep their owner until
 * the ring retires them (tbv_path_cancel_data_match()). error_work holds a QP
 * reference of its own, so it is flushed (run to completion), never canceled.
 * Sleeps.
 */
static void tbv_qp_stop_producers(struct tbv_qp *tqp)
{
...
int tbv_apple_path_quiesce(struct tbv_rail *rail, struct tbv_qp *tqp,
			   const char *reason, unsigned long deadline)
{
	struct tbv_path *path = &rail->path;
	struct tbv_state *state = rail->peer ? rail->peer->state : NULL;
	struct tbv_qp *owner = tqp;
	unsigned long start = jiffies;
	unsigned long flags;
	unsigned int refs_target = 1;
	...
	/*
	 * One quiesce at a time per rail: a rail removal can overlap a QP
	 * destroy, and the ring barrier must not be issued twice.
	 */
	mutex_lock(&rail->quiesce_lock);

	/* 1. Close admission and successor binding. */
	spin_lock_irqsave(&rail->bind_lock, flags);
	was_quarantined = rail->bind.state == TBV_RAIL_QUARANTINED;
	if (!was_quarantined)
		WRITE_ONCE(rail->bind.state, TBV_RAIL_QUIESCING);
	rail->bind.quiesce_count++;
	if (!owner && rail->bind.qp && tbv_qp_get_live(rail->bind.qp)) {
		/*
		 * The bound QP is still alive (rail removal, module exit):
		 * we are its producer stop. A QP that is already closing is
		 * running this primitive from its own destroy.
		 */
		owner = rail->bind.qp;
		owner_ref = true;
		refs_target = 2;
	...
	spin_unlock_irqrestore(&rail->bind_lock, flags);
	tbv_path_close_tx(path);

	/* 2. Stop producers. */
	if (owner) {
		if (owner_ref)
			tbv_qp_mark_error(owner);
		tbv_qp_stop_producers(owner);
	}

	/* 3. Cancel unsent packets; submitted DMA keeps its owner until retired. */
	tbv_path_cancel_queued_tx(path);

	/*
	 * 4. Short local drain, then the ring-stop barrier.
	 *
	 * Every descriptor still outstanding here belongs to a WR that step 2
	 * has already failed (a completed WR has no unretired frame), so the
	 * only thing worth waiting for is a descriptor the peer is consuming
	 * right now. A descriptor a starved peer would consume LATER must not
	 * be waited on: it would be transmitted into the peer's successor QP.
	 * Measured 2026-09-06 on zeus: three E2E-starved frames left in the
	 * ring by a destroy were sent into the next Mac QP the moment it
	 * posted receives and corrupted its first message (LOC_LEN_ERR,
	 * byte_len=4096). Hence the tight cap; @deadline bounds the reference
	 * wait after the barrier.
	 */
	drain_left = (long)deadline - (long)jiffies;
	if (drain_left < 0)
		drain_left = 0;
	ret = tbv_path_drain_tx(path,
				jiffies + min_t(long, drain_left,
						msecs_to_jiffies(TBV_QUIESCE_DRAIN_MS)));
	if (ret) {
		struct tbv_path_ring_snapshot snap;

		restart = !rail->removing && state &&
			  READ_ONCE(state->verbs_registered);
		tbv_path_tx_ring_snapshot(path, &snap);
		pr_warn("apple rail %u quiesce(%s): %d TX frames unretired after %u ms (data_tx_posted=%lld ring head=%d tail=%d hw_cons=%d outstanding=%d tail_completed=%u hw_options=0x%08x); stopping the ring pair so the NHI cannot transmit them into a successor%s\n",
			rail->rail_id, reason, tbv_path_unretired_tx(path),
			jiffies_to_msecs(jiffies - start),
			atomic64_read(&path->data_tx_posted), snap.head,
			snap.tail, snap.hw_index, snap.outstanding,
			snap.tail_completed, snap.hw_options,
			restart ? "; restarting the pair" : "");
		barrier = true;
		ret = tbv_path_ring_barrier(path, restart);
		spin_lock_irqsave(&rail->bind_lock, flags);
		rail->bind.ring_stops++;
		spin_unlock_irqrestore(&rail->bind_lock, flags);
	}
	if (!ret && owner) {
		/*
		 * After a barrier every ring callback has already run
		 * synchronously inside tb_ring_stop(); only deferred completion
		 * work can still hold a reference, and it does not wait on the
		 * transport, so a short grace is enough. Without a barrier the
		 * path retired on its own and the full deadline applies.
		 */
		ret = tbv_qp_wait_refs(owner, refs_target,
				       barrier ? jiffies + msecs_to_jiffies(TBV_QUIESCE_BARRIER_GRACE_MS) :
						 deadline);
		if (ret) {
			pr_err("apple rail %u quiesce(%s): qpn %u still holds %u refs with %d TX frames unretired (barrier=%u); a non-ring holder has not released it\n",
			       rail->rail_id, reason, owner->base.qp_num,
			       refcount_read(&owner->refs),
			       tbv_path_unretired_tx(path), barrier);
			ret = -EIO;
		}
	}

	/*
	 * 5/6. Record the outcome and release the binding. Quarantine is rail
	 * state: no verbs object (QP, GID) is held to represent a poisoned path.
	 */
	spin_lock_irqsave(&rail->bind_lock, flags);
	rail->bind.last_teardown_error = ret;
	rail->bind.last_teardown_ms = jiffies_to_msecs(jiffies - start);
	rail->bind.last_teardown_reason = reason;
	if (owner) {
		if (rail->bind.qp == owner)
			rail->bind.qp = NULL;
		owner->rail_bound = false;
	}
	if (!ret && !(barrier && restart)) {
		/*
		 * A clean retirement never lifts an existing quarantine: only
		 * a new peer session may (tbv_rail_apple_admit_locked()).
		 */
		if (!was_quarantined)
			WRITE_ONCE(rail->bind.state, TBV_RAIL_UNBOUND);
	} else {
		WRITE_ONCE(rail->bind.state, TBV_RAIL_QUARANTINED);
		rail->bind.quarantine_rx_mark =
			atomic64_read(&path->data_rx_completed);
		rail->bind.quarantine_since = jiffies;
		rail->bind.quarantine_reason = reason;
		rail->bind.quarantine_epoch = rail->bind.epoch;
	}
	reopen = rail->bind.state == TBV_RAIL_UNBOUND && !barrier &&
		 !rail->removing;
	spin_unlock_irqrestore(&rail->bind_lock, flags);
	if (reopen)
		tbv_path_open_tx(path);
	if (owner_ref)
		tbv_qp_put(owner);

	pr_info("apple rail %u quiesced (%s) in %u ms: state=%s barrier=%u err=%d unretired=%d\n",
		rail->rail_id, reason, jiffies_to_msecs(jiffies - start),
		tbv_rail_bind_state_name(READ_ONCE(rail->bind.state)), barrier,
		ret, tbv_path_unretired_tx(path));
	mutex_unlock(&rail->quiesce_lock);
	return ret;
}
			 ...
			 int attr_mask, struct ib_udata *udata)
{
	...
	if (activate_apple_tunnel) {
		...
		if (ret)
			return ret;
		/*
		 * Bind the rail before the transition commits: a QP that is
		 * allowed into RTR/RTS on an unbound or mismatched path would
		 * transmit its first frame before anyone could refuse it.
		 */
		ret = tbv_qp_apple_bind_rail(tqp, attr_mask & IB_QP_DEST_QPN,
					     attr->dest_qp_num);
		if (ret) {
	...
	if (attr_mask & IB_QP_AV) {
		/*
		 * Shallow copy. attr->ah_attr.grh.sgid_attr is a GID table
		 * reference owned by the RDMA core (qp->av_sgid_attr); this copy
		 * never acquired one, so it must not carry the pointer where a
		 * later query could hand it back or a teardown could put it.
		 */
		tqp->attr.ah_attr = attr->ah_attr;
		tqp->attr.ah_attr.grh.sgid_attr = NULL;
	}
...
out_unlock:
	...
	if (deactivate_apple_tunnel) {
		/*
		 * Leaving RTR/RTS/SQD/SQE ends this epoch on the rail: retire
		 * its DMA now so a later RESET->INIT->RTR binds a fresh epoch
		 * on a path with nothing of the old one in flight.
		 */
		if (tqp->rail && tbv_qp_owns_apple_rail(tqp))
			tbv_apple_path_quiesce(tqp->rail, tqp, "modify_qp",
					       tbv_ibdev_quiesce_deadline());
		tbv_qp_apple_unbind_rail(tqp);
		tbv_qp_release_apple_tunnel(tqp);
	}
			...
			int attr_mask, struct ib_qp_init_attr *init_attr)
{
	...
	if (attr) {
		*attr = tqp->attr;
		...
		/* Borrowed GID references are never reported back. */
		attr->ah_attr.grh.sgid_attr = NULL;
		attr->alt_ah_attr.grh.sgid_attr = NULL;
	}
	if (init_attr)
...
{
	struct workqueue_struct *dedicated =
		tqp->rail ? tbv_path_tx_wq(&tqp->rail->path) : NULL;
	struct workqueue_struct *wq = tqp->owner && tqp->owner->workqueue ?
	...
	/*
	 * Default (tx_worker_dedicated=0, tx_worker_cpu=-1) is byte-identical
	 * to what this always did: the shared unbound WQ_HIGHPRI device
	 * workqueue. With a dedicated per-rail bounded workqueue,
	 * queue_work_on() places the post half on a named CPU -- the only half
	 * that CAN be placed, since the reap/CQ chain rides the NHI interrupt.
	 */
	if (dedicated) {
		cpu = tbv_path_tx_wq_cpu();
		if (cpu >= 0)
			queue_work_on(cpu, dedicated, &tqp->apple_sq_work);
		else
			queue_work(dedicated, &tqp->apple_sq_work);
		return;
	}
...
static bool tbv_send_complete(struct tbv_send_ctx *send, int status)
{
	...
	/*
	 * Kernel CQ insertion time. Stamped before the CQ push so the interval
	 * "last frame reaped -> kernel CQ" excludes nothing and includes no
	 * application polling: the bench-observed post->completion latency is
	 * this stamp plus the application's own CQ handling, which is exactly
	 * the separation Astra v12 A4 asks for.
	 */
	tbv_txtrace_stamp(&send->tx_stamps, TBV_TX_STAGE_CQ);
	send->tx_stamps.cpu_cq = raw_smp_processor_id();
	if (send->tx_stamps.active && tqp->rail)
		tbv_txtrace_record(tqp->rail->path.tx_trace, &send->tx_stamps,
				   send->wr_id, tqp->base.qp_num, send->psn,
				   status);
	...
	while (!list_empty(complete)) {
		...
		if (!status) {
			...
			if (delay_us) {
				...
				/*
				 * The delay is jiffies-granular: any non-zero
				 * value costs AT LEAST one jiffy (1-2 ms at
				 * CONFIG_HZ=1000), never delay_us. Say so once
				 * so a microsecond-looking knob is not read as
				 * a microsecond-scale delay in a latency run.
				 */
				pr_warn_once("apple_tx_completion_delay_us=%u rounds up to >= 1 jiffy (%lu jiffies, ~%u ms); completions are NOT delayed by microseconds\n",
					     delay_us, delay,
...
static void tbv_qp_timeout_work(struct work_struct *work)
{
	...
	/*
	 * Move the QP to ERR BEFORE completing the timed-out sends. Completing
	 * one releases its Apple SQ slot and TX window, and while the QP is
	 * still RTS the SQ worker takes that as admission for the next queued
	 * WR and submits its first frame group to a path that just proved it
	 * does not complete. Measured 2026-09-06: tx_unretired 2 at the timeout
	 * log, 6 by the destroy quiesce (one more group of
	 * apple_tx_max_inflight_frames). ERR first closes that window;
	 * tbv_qp_mark_error() flushes pending sends but the timed-out ones were
	 * already taken off pending_sends above, so they complete exactly once
	 * below with their -ETIMEDOUT/-EAGAIN status.
	 */
	if (tx_timed_out) {
		if (tqp->rail) {
			struct tbv_path *path = &tqp->rail->path;
			struct tbv_path_ring_snapshot snap;

			tbv_path_tx_ring_snapshot(path, &snap);
			pr_warn_ratelimited("QP %u: send WR timed out (%u ms) on rail %u with %d TX frames unretired (data_tx_posted=%lld); ring running=%u head=%d tail=%d hw_cons=%d outstanding=%d sw_queued=%d tail_completed=%u hw_options=0x%08x; QP -> ERR\n",
					    tqp->base.qp_num,
					    jiffies_to_msecs(timeout),
					    tqp->rail->rail_id,
					    tbv_path_unretired_tx(path),
					    atomic64_read(&path->data_tx_posted),
					    snap.running, snap.head, snap.tail,
					    snap.hw_index, snap.outstanding,
					    snap.sw_queued, snap.tail_completed,
					    snap.hw_options);
		}
		tbv_qp_mark_error(tqp);
	}
...
static void tbv_apple_send_tx_done(void *ctx, int status)
{
	...
	tbv_txtrace_stamp_once(&send->tx_stamps, TBV_TX_STAGE_FIRST_CB);
	tbv_txtrace_stamp(&send->tx_stamps, TBV_TX_STAGE_LAST_CB);
	send->tx_stamps.cpu_reap = raw_smp_processor_id();
				     ...
				     tbv_path_tx_done_fn done, void *done_ctx)
{
	...
	/*
	 * Submission trace for the producer-stop audit: a frame handed to the
	 * path after the QP left RTS is a submission after producers were
	 * supposed to be stopped (measured 2026-09-06: unretired grew 2 -> 6
	 * between the WR-timeout log and the destroy quiesce).
	 */
	if (READ_ONCE(tqp->state) != IB_QPS_RTS || READ_ONCE(tqp->closing))
		pr_warn_ratelimited("QP %u: apple frame submitted in qp_state=%d closing=%u (path data_tx_posted=%lld)\n",
				    tqp->base.qp_num, READ_ONCE(tqp->state),
				    READ_ONCE(tqp->closing),
				    atomic64_read(&path->data_tx_posted));
...
static int tbv_post_apple_send(struct tbv_qp *tqp, const struct ib_send_wr *wr)
{
	...
	u64 post_entry_ns = tbv_txtrace_now();
	unsigned long flags;
	u32 total_len = 0;
	int nsegs = 0;
	void *payload = NULL;
	...
	if (post_entry_ns) {
		ctx->tx_stamps.active = true;
		ctx->tx_stamps.t[TBV_TX_STAGE_POST_ENTRY] = post_entry_ns;
		ctx->tx_stamps.length = total_len;
		ctx->tx_stamps.frames =
			DIV_ROUND_UP(total_len, TBV_APPLE_FRAME_SIZE);
		ctx->tx_stamps.cpu_post = raw_smp_processor_id();
	}
	...
	/*
	 * Stamped BEFORE the handoff, not after the function returns: once the
	 * entry is queued the SQ worker owns the context and may complete it,
	 * so a later write here would race the trace record. The queue_work()
	 * call itself therefore lands in the dispatch interval, which is where
	 * it belongs.
	 */
	tbv_txtrace_stamp(&ctx->tx_stamps, TBV_TX_STAGE_POST_HANDOFF);
			      ...
			      const void *payload, u32 len, u8 sof, u8 eof)
{
	...
	/*
	 * Generation boundary: while the rail is retiring a previous epoch
	 * (or quarantined after a forced ring stop) nothing arriving on it
	 * may reach a QP, be assembled into a pending message or consume a
	 * WQE. Apple frames carry no incarnation, so this is decided on the
	 * path before QP lookup. Unlocked read on purpose: the state flips
	 * under bind_lock, and the RX path only needs a monotonic hint.
	 */
	if (path->rail) {
		enum tbv_rail_bind_state bind_state =
			READ_ONCE(path->rail->bind.state);

		if (bind_state == TBV_RAIL_QUIESCING ||
		    bind_state == TBV_RAIL_QUARANTINED) {
			atomic64_inc(&state->apple_rx_quarantine_dropped);
			return;
		}
	}
					       ...
					       int *ret)
{
	...
	unsigned int max_frames = tbv_apple_tx_budget();
	u32 frame_charge = tbv_apple_tx_frame_charge(frames,
						     tbv_apple_tx_group());
	bool exclusive = tbv_apple_tx_exclusive(frames);
	unsigned long flags;
	bool acquired = false;
	...
	*frames_acquired = 0;
	...
	for (;;) {
		...
		*frames_acquired = 0;
		...
		/*
		 * Data-queue shortage. This used to be msleep(1); at
		 * CONFIG_HZ=1000 that is a 1-2 ms sleep, which is exactly the
		 * band of the size-, depth- and rate-independent maximum seen
		 * in 38 of 39 cells of the 2026-09-06 timing matrix (2.4-4.6
		 * ms, including 4 KiB at depth 1 where p50 is 25 us). Wait on
		 * the event instead: the path wakes us the moment a descriptor
		 * reaches the ring, a reservation is released, or admission
		 * closes. The wait holds the path reference so the path cannot
		 * go away underneath it, and is bounded by an internal
		 * safety-net timeout that is counted, never used as pacing.
		 */
		tbv_path_wait_data_space(path, reserve_frames);
		tbv_release_path_refs(&path, 1);
	}
}
				 ...
				 struct tbv_apple_sq_entry *entry)
{
	...
	u32 posted = 0;
	u32 group_posted = 0;
	u32 group_limit = tbv_apple_tx_group();
	bool raw_mode = tbv_path_apple_tx_raw_mode();
	bool sent_any = false;
	int ret;

	tbv_txtrace_stamp(&ctx->tx_stamps, TBV_TX_STAGE_WORKER_START);
	ctx->tx_stamps.cpu_worker = raw_smp_processor_id();
	...
	while (offset < entry->length) {
		...
		if (raw_mode) {
			...
			ret = tbv_post_apple_send_frame(
				...
				payload_len, tbv_apple_tx_sof(offset, nfrags),
				last ? 3 : 2, tbv_apple_send_tx_done, ctx);
			if (!ret) {
				remaining--;
				posted++;
			}
		...
		tbv_txtrace_stamp_once(&ctx->tx_stamps,
				       TBV_TX_STAGE_FIRST_PUBLISH);
		tbv_txtrace_stamp(&ctx->tx_stamps, TBV_TX_STAGE_LAST_PUBLISH);
		group_posted++;
		sent_any = true;
		offset += payload_len;
...
static void tbv_ibdev_quiesce_all_rails(struct tbv_state *state)
{
	for (;;) {
		struct tbv_rail *target = NULL;
		struct tbv_peer *peer;
		...
		list_for_each_entry(peer, &state->peers, node) {
			...
			list_for_each_entry(rail, &peer->rails, node) {
				if (rail->stop_quiesced)
					continue;
				rail->stop_quiesced = true;
				refcount_inc(&rail->refcnt);
				target = rail;
				break;
			}
		...
		if (!target)
			return;
		tbv_apple_path_quiesce(target, NULL, "module_exit",
				       tbv_ibdev_quiesce_deadline());
		tbv_rail_put(target);
	}
}
...
void tbv_ibdev_stop(struct tbv_state *state)
{
	...
	/*
	 * Stop producers and retire DMA on every rail BEFORE flushing the
	 * workqueue and before ib_unregister_device(): otherwise the flush
	 * waits on SQ workers that wait on transport progress, and unregister
	 * waits on destroy_qp callbacks that wait on references only a ring
	 * barrier (which used to live behind unregister) could release.
	 */
	tbv_ibdev_quiesce_all_rails(state);
	if (state->workqueue)
