...
static int tbv_debugfs_summary_show(struct seq_file *s, void *unused)
{
	...
	seq_printf(s, "apple_rx_quarantine_dropped: %lld\n",
		   atomic64_read(&state->apple_rx_quarantine_dropped));
	seq_printf(s, "apple_tx_unbound: %lld\n",
		   atomic64_read(&state->apple_tx_unbound));
	seq_printf(s, "apple_bind_refused: %lld\n",
		   atomic64_read(&state->apple_bind_refused));
	...
	list_for_each_entry(peer, &state->peers, node) {
		...
		list_for_each_entry(rail, &peer->rails, node) {
			...
			seq_printf(s,
				   ...
				   rail->remote_rx_hop);
			seq_printf(s,
				   "    data_tx_enqueued=%lld data_tx_posted=%lld data_tx_completed=%lld data_tx_canceled=%lld data_tx_credit_stalls=%lld data_tx_credit_received=%lld tx_credits=%u/%u\n",
				   atomic64_read(&rail->path.data_tx_enqueued),
				   atomic64_read(&rail->path.data_tx_posted),
				   atomic64_read(&rail->path.data_tx_completed),
				   atomic64_read(&rail->path.data_tx_canceled),
			...
			tbv_ibdev_debugfs_rail_show(s, rail);
			seq_printf(s,
			...
			seq_printf(s,
				   "    tx_space_wait waits=%lld timeouts=%lld total_ns=%lld max_ns=%lld\n",
				   atomic64_read(&rail->path.tx_space_waits),
				   atomic64_read(&rail->path.tx_space_wait_timeouts),
				   atomic64_read(&rail->path.tx_space_wait_ns),
				   atomic64_read(&rail->path.tx_space_wait_max_ns));
			seq_printf(s,
			...
			tbv_path_show_apple_throttle(s, &rail->path);
			seq_printf(s,
				   "    tx_worker dedicated=%u pinned_cpu=%d tx_last_cb_cpu=%d\n",
				   !!tbv_path_tx_wq(&rail->path),
				   tbv_path_tx_wq_cpu(),
				   READ_ONCE(rail->path.tx_last_cb_cpu));
			tbv_txtrace_show_summary(s, rail->path.tx_trace);
		}
	}
...
/*
 * Raw per-WR TX stage records: the worst TBV_TX_TRACE_WORST by total first
 * (so a single multi-millisecond event can be read end to end without
 * searching), then the ring oldest-first. Every stamp is printed as an offset
 * from that WR's own post entry, plus the derived intervals.
 */
static int tbv_debugfs_tx_trace_show(struct seq_file *s, void *unused)
{
	struct tbv_state *state = s->private;
	struct tbv_peer *peer;

	seq_puts(s,
		 "# stages: post_entry post_handoff worker_start first_publish last_publish first_cb last_cb cq\n"
		 "# first_cb/last_cb are the first/last TX completion CALLBACKS observed, an\n"
		 "# observation bound on the hardware interrupt, not IRQ entry: the Apple NHI\n"
		 "# handler does schedule_work(&ring->work) and ring_work() runs our callbacks.\n"
		 "# offsets are ns relative to that WR's post_entry; '-' means the stage was\n"
		 "# never reached or the interval had no valid sample.\n");
	...
	list_for_each_entry(peer, &state->peers, node) {
		...
		list_for_each_entry(rail, &peer->rails, node) {
			...
			snprintf(label, sizeof(label), "peer%u/rail0x%x",
				 peer->peer_id, rail->rail_id);
			tbv_txtrace_show_records(s, rail->path.tx_trace, label);
		}
	}
	mutex_unlock(&state->lock);
...
DEFINE_SHOW_ATTRIBUTE(tbv_debugfs_tx_trace);
...
int tbv_debugfs_init(struct tbv_state *state)
{
	...
	debugfs_create_file("tx_trace", 0444, state->debugfs_dir, state,
			    &tbv_debugfs_tx_trace_fops);
	return 0;
}
