...
/*
 * How a TX stall episode ended. The ladder is a LIVENESS fallback only: a
 * degraded transfer that keeps going beats a dead run, but nothing here is
 * required for correctness.
 *
 *   PEER    the peer resumed on its own (no rung fired)
 *   REAP    T1 found descriptors the NHI had completed but never notified
 *   REKICK  T2 re-announced the producer index, and progress followed
 *   TIMEOUT T3 gave up: quiesce, barrier, quarantine (the old only outcome)
 */
enum tbv_tx_stall_recovery {
	TBV_TX_STALL_RECOVERY_PEER,
	TBV_TX_STALL_RECOVERY_REAP,
	TBV_TX_STALL_RECOVERY_REKICK,
	TBV_TX_STALL_RECOVERY_TIMEOUT,
	TBV_TX_STALL_RECOVERY_COUNT,
};
...
struct tbv_path {
	...
	/* Woken when tx_inflight reaches zero; see tbv_path_drain_tx(). */
	wait_queue_head_t tx_retire_wait;
	/*
	 * Woken when TX data-queue occupancy drops or admission changes, i.e.
	 * whenever tbv_path_reserve_data() may stop returning -ENOMEM. Replaces
	 * the msleep(1) retry that used to sit on the Apple SEND path: at
	 * CONFIG_HZ=1000 that sleep is 1-2 ms, which is exactly the band of the
	 * size- and depth-independent maximum measured in every cell of the
	 * 2026-09-06 timing matrix.
	 */
	wait_queue_head_t tx_space_wait;
	/* How often the SEND path had to wait for data-queue space, and how long. */
	atomic64_t tx_space_waits;
	atomic64_t tx_space_wait_ns;
	atomic64_t tx_space_wait_max_ns;
	/* Safety-net firings: the wait timed out instead of being woken. */
	atomic64_t tx_space_wait_timeouts;
	/* Per-WR TX stage timestamps for this rail (NULL when disabled). */
	struct tbv_tx_trace *tx_trace;
	/*
	 * Dedicated bounded WQ_HIGHPRI worker for the TX post half, or NULL to
	 * use the shared unbound device workqueue (the default). Only the post
	 * half can be placed: the reap/CQ chain runs in ring_work() on whatever
	 * CPU the NHI interrupt landed on, and AIC2 cannot steer interrupts.
	 */
	struct workqueue_struct *tx_wq;
	/* CPU the most recent TX completion callback ran on (observation only). */
	int tx_last_cb_cpu;
	/*
	 * TX completion watchdog: armed while descriptors are outstanding,
	 * warns once (with the ring's own indices) when none retires for
	 * TBV_TX_STALL_WARN_MS and flags the path stalled until one does.
	 */
	struct delayed_work tx_watchdog_work;
	unsigned long tx_watchdog_last_progress;
	/* jiffies of the most recent TX retirement (written per completion). */
	unsigned long tx_last_retire;
	u64 tx_watchdog_last_retired;
	u32 tx_stall_count;
	/* Stalls that resumed on their own; duration = last retirement before
	 * the stall -> first retirement after it (the Mac's credit-return gap). */
	u32 tx_stall_recovered;
	u32 tx_stall_longest_ms;
	u64 tx_stall_total_ms;
	u32 tx_stall_frames;
	bool tx_stalled;
	/*
	 * Which rung of the recovery ladder actually ended each episode. None of
	 * the rungs owns correctness -- the credit ledger, the QP send list and
	 * the retirement barrier do -- so this is how we learn whether a rung is
	 * worth keeping at all.
	 */
	u32 tx_stall_recovered_by[TBV_TX_STALL_RECOVERY_COUNT];
	/* T2 producer-doorbell re-kicks issued, and how many preceded a recovery. */
	u32 tx_rekicks;
	/* One re-kick per episode; cleared when the episode ends. */
	bool tx_stall_rekicked;
	/* Submitted-but-unretired TX descriptors (control + data). */
	atomic_t tx_inflight;
	atomic64_t data_tx_enqueued;
	atomic64_t data_tx_posted;
	atomic64_t data_tx_completed;
	/* Data descriptors retired by ring cancellation instead of completion. */
	atomic64_t data_tx_canceled;
	/* TX descriptors reaped from the ring tail by a quiesce (missed IRQ). */
	atomic64_t tx_reaped;
	...
	bool tx_scheduling;
	/* Quiesce closed admission: no enqueue/reserve/schedule (tx_lock). */
	bool tx_closed;
	/* Ring pair stopped by tbv_path_ring_barrier(); destroy skips its stop. */
	bool rings_stopped;
...
/*
 * Rail/path binding state for the Apple-compatible transport.
 *
 * FA57 frames carry no connection incarnation: the peer cannot tell a frame of
 * the previous QP from a frame of the next one, and neither can we. The rail
 * therefore records which QP epoch owns its path and refuses a successor until
 * the previous epoch has retired every submitted DMA descriptor:
 *
 *   UNBOUND -> BOUND(epoch, local_qpn, remote_qpn) -> QUIESCING
 *           -> UNBOUND                 (drain verified locally)
 *           -> QUARANTINED -> UNBOUND  (drain needed a ring-stop barrier; a
 *                                       later bind lifts it only once the peer
 *                                       has been observed silent)
 *
 * Every refusal is a teaching error in dmesg, never a silent transmit on an
 * incompatible path. Protected by tbv_rail.bind_lock. @qp is valid while the
 * state is BOUND or QUIESCING and is cleared before that QP is freed.
 */
enum tbv_rail_bind_state {
	TBV_RAIL_UNBOUND,
	TBV_RAIL_BOUND,
	TBV_RAIL_QUIESCING,
	TBV_RAIL_QUARANTINED,
};

struct tbv_qp;

struct tbv_rail_bind {
	enum tbv_rail_bind_state state;
	struct tbv_qp *qp;
	u64 epoch;
	u32 local_qpn;
	u32 remote_qpn;
	bool remote_qpn_known;
	u64 quiesce_count;
	u64 ring_stops;
	u64 bind_refused;
	int last_teardown_error;
	u32 last_teardown_ms;
	const char *last_teardown_reason;
	/* path.data_rx_completed sampled when the quarantine was entered/re-armed */
	u64 quarantine_rx_mark;
	unsigned long quarantine_since;
	/* Why and at which epoch the rail was quarantined ("none" if never). */
	const char *quarantine_reason;
	u64 quarantine_epoch;
};
...
struct tbv_rail {
	...
	struct tbv_rail_bind bind;
	/* Serializes tbv_apple_path_quiesce() invocations on this rail. */
	...
	bool active;
	bool removing;
	/* tbv_ibdev_stop() already ran the quiesce primitive on this rail. */
	bool stop_quiesced;
	/*
...
struct tbv_state {
	...
	atomic64_t apple_rx_quarantine_dropped;
	atomic64_t apple_tx_unbound;
	atomic64_t apple_bind_refused;
...
void tbv_rail_bind_init(struct tbv_rail *rail);
const char *tbv_rail_bind_state_name(enum tbv_rail_bind_state state);
/* Per-rail binding + teardown diagnostics for the debugfs "peers" file. */
...
/*
 * Event-driven replacement for a resource-shortage sleep: block until
 * tbv_path_reserve_data(path, frames) can plausibly succeed (or until
 * admission closes, so the caller sees the real error). Bounded by an
 * internal safety-net timeout; never spins. The caller must still re-run
 * tbv_path_reserve_data() and re-check its own cancellation state.
 */
...
/*
 * TX ring state read from the NHI ring's own indices (not from module
 * counters): descriptors handed to hardware and not yet reaped by software,
 * frames still on the NHI software queue, and whether the oldest submitted
 * descriptor already carries RING_DESC_COMPLETED (a reaped-vs-stalled
 * discriminator).
 */
struct tbv_path_ring_snapshot {
	int head;
	int tail;
	int size;
	int outstanding;
	int sw_queued;
	int in_flight;
	/* Full 12-bit flags of the oldest submitted descriptor. */
	u32 tail_flags;
	/* Hardware index register: consumer (TX) / producer (RX), or -1. */
	int hw_index;
	/* Ring options register readback (valid/E2E bits), or 0 if unknown. */
	u32 hw_options;
	/* Ring flags as allocated (RING_FLAG_E2E, RING_FLAG_FRAME, ...). */
...
/* Quiesce building blocks (path side); see tbv_apple_path_quiesce(). */
...
/*
 * Allocation-independent quiesce of a rail's data path, used by
 * tbv_destroy_qp(), tbv_peer_remove_rail() and tbv_ibdev_stop(): close
 * admission and successor binding, stop producers, cancel unsent packets,
 * drain submitted DMA within @deadline, and fall back to a tb_ring_stop()
 * barrier. @tqp is the QP being destroyed (its refs are part of the drain
 * predicate) or NULL to act on whatever QP the rail is bound to. Returns 0
 * when no callback of the retired epoch can run anymore, -EIO when the NHI
 * barrier failed (reported, rail quarantined). Sleeps; call without locks.
 */
